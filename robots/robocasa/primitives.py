...
Extracted from RoboCasaDriver to separate concerns: toolkit = what the robot CAN do;
driver = how commands are dispatched and the file protocol loop.
"""
import json
import os
...
class RoboCasaPrimitives:
    def __init__(self, env, workdir, hi_res, vla_client):
        self.env = env
    ...
    def dump_state(self, step_idx):
        ...
        # waypoints. Wrapped so a render glitch degrades gracefully, not kills the driver.
        try:
            ...
            print(f"[driver] navcam render failed at step {step_idx}: {e}", flush=True)
