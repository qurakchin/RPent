...
def _load_states() -> list:
    """Return the parsed driver state trace from ``states.json``."""
...
def dump_state(
    driver: RoboCasaPrimitives,
    output_dir: str,
    step_idx: int,
    log: dict | None = None,
) -> dict:
    """Call ``driver.dump_state()`` then append to ``states.json`` with artifact paths.
    ...
    state = driver.dump_state(step_idx)

    # Build states.json blob with artifact paths
    nn = f"{step_idx:02d}"
    blob = {
        "step_idx": step_idx,
        "robocasa_terminated": driver.env.terminated,
...
def view_driver_state(step: int | None = None) -> dict:
    """Read driver state with embedded images and role-labeled image metadata."""
